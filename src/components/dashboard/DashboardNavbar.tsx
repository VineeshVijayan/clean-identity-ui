...
export const DashboardNavbar = ({ user, onMenuClick, onLogout }: DashboardNavbarProps) => {
  ...
  return (
    <header className="relative h-16 border-b border-border bg-card flex items-center justify-between px-4 sm:px-6">      <div className="flex items-center gap-4">
      <button
      ...
      <div className="absolute left-1/2 transform -translate-x-1/2 hidden sm:block">
        <span className="text-xl font-medium text-foreground">
          Welcome, <span className="text-primary font-semibold text-xl">{user?.name || "User"}</span>
        </span>
      </div>
      {/* Search bar removed */}
    </div>
